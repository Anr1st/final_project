...
Для обмена данными между клиентом и сервером используются объекты передачи данных (DTO). DTO отделяют внутренние модели базы данных от внешних контрактов: клиент не видит лишних полей (например, хэш пароля), а сервер принимает только те данные, которые ожидает. Основные DTO системы представлены в таблице 10.

Таблица 10 - Объекты передачи данных (DTO)

| DTO                  | Назначение                                  | Поля |
|----------------------|---------------------------------------------|------|
| RegisterRequest      | Тело запроса на регистрацию                 | email, password, name, role |
| LoginRequest         | Тело запроса на вход                        | email, password |
| AuthResponse         | Ответ при регистрации                       | id, email, fullName, role, isVerified, token |
| LoginResponse        | Ответ при входе                             | token |
| UpdateProfileRequest | Изменение профиля                           | email, name |
| CreatePropertyDto    | Создание объявления                         | title, city, address, propertyType, rooms, basePrice, description |
| UpdatePropertyDto    | Редактирование объявления (все поля опциональны) | title, city, address, propertyType, rooms, basePrice, description |
| UpdateStatusDto      | Смена статуса объекта                       | status |
| AddPhotoDto          | Добавление фотографии                       | url, isCover |
| AddPriceRateDto      | Добавление ценового тарифа                  | startDate, endDate, pricePerDay |
| CreateBookingRequest | Создание бронирования                       | propertyId, checkIn, checkOut, comment |
| BookingResponse      | Ответ с деталями бронирования               | id, propertyId, renterId, checkIn, checkOut, totalPrice, serviceFee, status, comment, createdAt, payment |
| ProcessPaymentRequest| Запрос на оплату                            | bookingId |
| PaymentResponse      | Ответ с деталями платежа                    | id, bookingId, amount, status, transactionDate |

**3.4 Модели данных**

Модели данных - это C#-классы, которые Entity Framework Core отображает на таблицы PostgreSQL. Каждый класс соответствует одной сущности из этапа 1. Перечисления (enum) хранятся в базе как целые числа. Связи между моделями заданы навигационными свойствами и настроены в методе OnModelCreating класса AppDbContext. Соответствие моделей и таблиц представлено в таблице 11.

Таблица 11 - Модели данных и их таблицы

| Модель        | Таблица         | Ключевые поля и перечисления |
|---------------|-----------------|------------------------------|
| User          | Users           | Id (Guid, PK), Email (уникальный), PasswordHash, FullName, Role (guest/tenant/host/admin), IsVerified, IsBlocked, CreatedAt |
| Property      | Properties      | Id (Guid, PK), OwnerId (FK->User), Title, City, Address, PropertyType (apartment/house/room/studio), Rooms, BasePrice, Status (draft/active/inactive/blocked), Description, CreatedAt |
| Booking       | Bookings        | Id (Guid, PK), PropertyId (FK), RenterId (FK), CheckIn, CheckOut, TotalPrice, ServiceFee, Status (pending/confirmed/completed/cancelled), Comment, CreatedAt |
| Payment       | Payments        | Id (Guid, PK), BookingId (FK, уникальный, связь 1:1), Amount, Status (pending/succeeded/failed/refunded), TransactionDate |
| PriceRate     | PriceRates      | Id (Guid, PK), PropertyId (FK), StartDate, EndDate, PricePerDay |
| PropertyPhoto | PropertyPhotos  | Id (Guid, PK), PropertyId (FK), Url, IsCover |

Ограничения и связи, заданные на уровне базы:
- уникальный индекс на Users.Email - гарантирует уникальность почты;
- внешний ключ Properties.OwnerId -> Users.Id;
- внешние ключи Bookings.PropertyId -> Properties.Id и Bookings.RenterId -> Users.Id;
- связь один-к-одному Payments.BookingId -> Bookings.Id (уникальный индекс на BookingId);
- каскадное удаление: при удалении объекта удаляются его фотографии и тарифы.
...
необходимости заменить его без изменения остального кода. Заглушки зарегистрированы в контейнере зависимостей (Program.cs) и внедряются в контроллеры через конструктор. Поведение моков детерминировано: они всегда возвращают успех, что позволяет воспроизводимо тестировать основной сценарий. Описание моков приведено в таблице 12.

Таблица 12 - Моки внешних сервисов

| Мок                     | Какую внешнюю систему заменяет | Поведение |
|-------------------------|--------------------------------|-----------|
| MockPaymentService      | Платёжный шлюз банка            | ProcessPayment всегда возвращает true (оплата успешна), RefundPayment всегда возвращает true (возврат успешен). Печатает строку в консоль. |
| MockEmailService        | Сервис email-рассылки          | Методы SendRegistrationEmail, SendBookingCreatedEmail, SendBookingConfirmedEmail, SendBookingCancelledEmail печатают текст письма в консоль. |
| MockVerificationService | Сервис верификации личности    | VerifyIdentity всегда возвращает true. Печатает строку в консоль. |

**3.6 Аутентификация**

Аутентификация построена на JSON Web Token (JWT). Схема работы:

1. При регистрации или входе сервер проверяет данные и формирует токен методом CreateToken. В токен (claims) записываются идентификатор пользователя, email, имя и роль. Токен подписывается симметричным ключом (алгоритм HMAC-SHA256) и действует 12 часов.
2. Клиент сохраняет токен и передаёт его в каждом защищённом запросе в заголовке Authorization в формате Bearer <токен>.
3. Промежуточное ПО JWT (настроено в Program.cs) при каждом запросе проверяет подпись, издателя (Issuer), аудиторию (Audience) и срок действия токена. При успешной проверке данные пользователя становятся доступны контроллеру через свойство User.
4. Эндпоинты, помеченные атрибутом [Authorize], доступны только при наличии действительного токена. Эндпоинты [Authorize(Roles = "admin")] доступны только администратору.

Пароли не хранятся в открытом виде: при регистрации пароль хэшируется библиотекой BCrypt, при входе введённый пароль сверяется с хэшем методом BCrypt.Verify.

**3.7 Обработка ошибок**

Ошибки обрабатываются на уровне контроллеров и возвращаются клиенту в виде стандартных кодов HTTP с текстовым описанием. Соответствие ситуаций и кодов приведено в таблице 13.

Таблица 13 - Коды ответов и ситуации

| Код | Метод ASP.NET     | Когда возвращается |
|-----|-------------------|--------------------|
| 200 | Ok                | Операция выполнена успешно |
| 201 | CreatedAtAction   | Создан новый ресурс (объект, бронирование) |
| 204 | NoContent         | Удаление выполнено, тело ответа пустое |
| 400 | BadRequest        | Неверные данные или нарушение правила (даты, пустые поля, попытка отменить завершённую бронь) |
| 401 | Unauthorized      | Нет токена или неверный пароль |
| 403 | Forbid / 403      | Действие запрещено ролью или пользователь не верифицирован/заблокирован |
| 404 | NotFound          | Объект, пользователь или бронь не найдены |
| 409 | Conflict          | Конфликт логики (email занят, даты пересекаются) |

**3.8 Документация API (Swagger)**

Документация генерируется автоматически библиотекой Swashbuckle (Swagger). После запуска проекта в режиме разработки она доступна по адресу http://localhost:5121/swagger. Интерфейс Swagger UI показывает все эндпоинты, их параметры, схемы запросов и ответов, а также позволяет отправлять запросы прямо из браузера. Для защищённых эндпоинтов в Swagger настроена кнопка Authorize: после ввода токена он автоматически подставляется в заголовок Authorization всех последующих запросов.

**3.9 Sequence-диаграммы**

Ниже описаны последовательности вызовов для трёх ключевых сценариев.

Сценарий 1 - Регистрация:
Клиент -> AuthController: POST /api/v1/auth/register
AuthController -> AppDbContext: проверка уникальности email
AuthController -> BCrypt: хэширование пароля
AuthController -> AppDbContext: сохранение пользователя
AuthController -> MockEmailService: отправка письма о регистрации
AuthController -> Клиент: 200 OK + токен

Сценарий 2 - Создание бронирования и оплата:
Клиент -> BookingsController: POST /api/v1/bookings
BookingsController -> AppDbContext: проверка верификации, объекта, пересечения дат
BookingsController -> AppDbContext: расчёт цены, сохранение брони (pending)
BookingsController -> MockEmailService: письмо арендодателю
BookingsController -> Клиент: 201 Created
Клиент -> PaymentsController: POST /api/v1/payments/process
PaymentsController -> MockPaymentService: ProcessPayment
PaymentsController -> AppDbContext: статус платежа succeeded, брони confirmed
PaymentsController -> MockEmailService: письмо об оплате обеим сторонам
PaymentsController -> Клиент: 200 OK

Сценарий 3 - Отмена с возвратом:
Клиент -> BookingsController: POST /api/v1/bookings/{id}/cancel
BookingsController -> AppDbContext: проверка прав, статуса (не completed)
BookingsController -> AppDbContext: статус брони cancelled
BookingsController -> MockPaymentService: RefundPayment (если бронь была оплачена)
BookingsController -> AppDbContext: статус платежа refunded
BookingsController -> MockEmailService: письмо об отмене
BookingsController -> Клиент: 200 OK
...
# 4.1 Назначение этапа

На четвёртом этапе к готовому backend (ASP.NET Core REST API) добавлен отдельный графический интерфейс (frontend) на Streamlit. Интерфейс не содержит бизнес-логики: он служит клиентом, который отправляет HTTP-запросы к backend и наглядно показывает результат. Такое разделение соответствует архитектуре системы: вся логика и проверки правил остаются на сервере, а frontend отвечает только за отображение и удобство демонстрации.

Синхронное взаимодействие реализовано напрямую: frontend через библиотеку requests вызывает эндпоинты API и дожидается ответа, после чего показывает его пользователю. Внешние зависимости (платёжный шлюз, email-сервис, верификация личности) по-прежнему заменены моками на стороне backend, поэтому весь основной сценарий воспроизводится без реальных внешних систем.

## 4.2 Стек технологий frontend

- Python - язык реализации интерфейса;
- Streamlit - фреймворк для построения веб-интерфейса из Python-кода;
- requests - библиотека для отправки HTTP-запросов к backend;
- urllib3 - используется для отключения проверки самоподписанного сертификата в демонстрационном режиме.

Зависимости зафиксированы в файле requirements.txt. Тема оформления (цвета, шрифт) задана в файле .streamlit/config.toml.

## 4.3 Архитектура взаимодействия frontend и backend

Frontend и backend - два независимых процесса, которые запускаются раздельно. Backend слушает HTTP-порт, frontend открывается в браузере и обращается к этому адресу. Адрес backend задаётся в левом меню интерфейса, что позволяет переключаться между разными запусками без правки кода.

Все запросы проходят через единую функцию api_request. Она формирует полный URL, при необходимости подставляет заголовок авторизации Authorization: Bearer <токен>, отправляет запрос выбранным методом (GET, POST, PUT) и возвращает три значения: признак успеха, код ответа HTTP и тело ответа. Любой ответ сохраняется в состоянии сессии, что используется во вкладке диагностики.

Авторизация работает так же, как и в backend. После регистрации или входа frontend извлекает JWT-токен из ответа и сохраняет его в состоянии сессии (st.session_state). Дальше токен автоматически прикладывается ко всем закрытым запросам. При выходе токен очищается. Поскольку Streamlit обращается к backend по схеме сервер-сервер через requests.

## 4.5 Заключение по этапу 4

В рамках четвёртого этапа система доработана до уровня рабочего прототипа с графическим интерфейсом. Реализован отдельный frontend на Streamlit, который синхронно взаимодействует с backend через REST API и наглядно демонстрирует основной сценарий: регистрацию, вход с получением JWT, управление профилем и верификацию, публикацию объекта, поиск и бронирование, оплату через мок и отмену с возвратом, а также администрирование пользователей. Внешние зависимости остаются изолированными за моками, бизнес-логика полностью сосредоточена на стороне backend. Поставленная цель этапа достигнута: получено работающее приложение, пригодное для демонстрации полного цикла работы сервиса.

# **ЗАКЛЮЧЕНИЕ**

В ходе работы спроектирован и реализован сервис посуточной аренды недвижимости. На первом этапе проработана доменная область: определены сущности, роли и бизнес-правила. На втором этапе спроектирован REST API: описаны эндпоинты, контракты в формате JSON и коды ошибок. На третьем этапе собран рабочий прототип на C# / ASP.NET Core с базой PostgreSQL, реализованы все эндпоинты, аутентификация через JWT, документация Swagger и моки трёх внешних сервисов. На четвёртом этапе реализован отдельный frontend на Streamlit. Поставленная цель достигнута: получено работающее приложение, демонстрирующее полный жизненный цикл бронирования.